...
export const BUFF_DMG_AMP_RATIO_ID = "buff.common.dmgAmpRatio" as const;

class DmgAmpRatioBuffDef extends BuffDef {
  constructor() {
    super({
      id: BUFF_DMG_AMP_RATIO_ID,
  ...
  override registerSimPlugins(registry: SimRegistry): void {
    registry.registerBuffDamageBonus({
      ...
      fn: ({ role, buff, type, collector }) => {
        ...
        if (delta !== 0) {
          collector.addValue("dmgAmpRatio", delta, `${this.name} (+${Math.round(delta * 100)}%)`);
        }
      },
    });
  }
...
export default new DmgAmpRatioBuffDef();
