...
export const BUFF_DMG_INC_RATIO_ID = "buff.common.dmgIncRatio" as const;

class DmgIncRatioBuffDef extends BuffDef {
  constructor() {
    super({
      id: BUFF_DMG_INC_RATIO_ID,
      name: "Damage Increase",
      icon: "BUFF_DMG_UP.png",
      durationFrames: -1,
      maxStacks: 99,
    });
  }
  ...
  override registerSimPlugins(registry: SimRegistry): void {
    registry.registerBuffDamageBonus({
      ...
      fn: ({ role, buff, type, collector }) => {
        ...
        if (delta !== 0) {
          collector.addValue("dmgIncRatio", delta, `${this.name} (+${Math.round(delta * 100)}%)`);
        }
      },
    });
  }
...
export default new DmgIncRatioBuffDef();
