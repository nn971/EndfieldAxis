...
export default function DamageStatisticPanel() {
  ...
  const operatorOptions = useMemo(() => {
    ...
    skillBoxes.forEach(box => ids.add(box.operatorId));
  ...
  return (
    <div className="rounded border border-zinc-700 bg-zinc-900 p-4">
      <div className="mb-4">
        <h2 className="text-lg font-semibold">Damage Statistics</h2>
        <div className="text-xs text-zinc-400">
          {hasData
            ? `Last run: ${simDamageCache.hitDamageSnapshots.length} hits`
            : 'Last run: -'}
        </div>
      </div>

      <div className="mb-4 rounded bg-zinc-800/50 p-3">
        <div className="mb-1 text-xs text-zinc-400">Total Damage</div>
        <div className="text-2xl font-bold">
          {hasData
            ? new Intl.NumberFormat().format(simDamageCache.totalDamage)
            : '(click Run)'}
        </div>
      </div>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-zinc-300">Watches</h3>
          <button
            type="button"
            onClick={() => dispatch(damageWatchAdded())}
        ...
        {damageWatches.length === 0 ? (
          <div className="text-xs italic text-zinc-500">
            No watches yet. Click "Add Watch" to track filtered damage.
          </div>
        ) : (
